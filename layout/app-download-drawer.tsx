...
// Returns true only when the app is running inside a real browser tab
// (not installed as a TWA/PWA/standalone APK wrapper).
function isRunningInBrowser() {
  ...
  // Installed PWA / TWA display modes
  const isStandaloneDisplay =
    window.matchMedia?.("(display-mode: standalone)").matches ||
    window.matchMedia?.("(display-mode: fullscreen)").matches ||
    window.matchMedia?.("(display-mode: minimal-ui)").matches;

  // Android TWA wraps the site and sets this referrer
...
export default function AppDownloadDrawer() {
  ...
  useEffect(() => {
    ...
    if (!isRunningInBrowser()) return; // installed app -> never show
