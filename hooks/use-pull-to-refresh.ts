...
/** Nearest ancestor (or the element itself) that actually scrolls, falling back to the document. */
...
export function usePullToRefresh({
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      startedAtTopRef.current = (scrollParentRef.current?.scrollTop ?? 0) === 0;
